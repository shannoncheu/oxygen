...
- 邀请注册：一个管理员管理账号，受邀用户各有独立空间，关闭公开注册。
...
从单账号版本更新后，原账号保留管理员身份，订阅、图片和密码继续使用。首次启动会自动迁移到多账号结构；迁移只执行一次。

使用下文的 Nginx 部署方式时，将更新命令中的 `docker compose pull db caddy` 改为 `docker compose pull db`。

已经使用 `compose.nginx.yaml` 的部署，可以直接执行：

```bash
cd /opt/oxygen
COMPOSE_FILE=compose.nginx.yaml bash scripts/backup.sh
git pull --ff-only
docker compose -f compose.nginx.yaml up -d --build --wait --wait-timeout 240
```
...
设置页导出的 JSON 只包含当前账号的业务数据、汇率设置、使用中的自定义头像和订阅图标，不包含账号密码、会话和邀请信息。恢复也只覆盖当前账号，不影响其他人。CSV 适合用表格查看订阅，不能用来完整恢复网站。服务器备份则包含所有账号的数据。

网页导出上限为 49 MiB、导入上限为 50 MiB，图片原始数据合计上限为 32 MiB。数据量超过限制时使用服务器备份。

## 忘记密码

普通用户联系管理员，在「设置 → 账号管理」生成密码重置链接和独立验证码。两项都需要提供给本人，有效期为 1 小时，只能使用一次。对方自行设置新密码后，旧密码和所有旧登录会话失效。

管理员忘记密码时，登录服务器，在项目目录执行：

```bash
docker compose exec app pnpm admin reset
```

按提示输入新密码。此命令只重置管理员，管理员的所有已登录设备都需要重新登录，不需要邮箱或短信验证。Nginx 部署可明确使用 `docker compose -f compose.nginx.yaml exec app pnpm admin reset`。
...
管理员在「设置 → 账号管理」创建邀请，填写对方使用的用户名和自己的当前密码。有效期可选 1、3 或 7 天，默认 7 天。

创建后分别复制注册链接和 8 位验证码，再分开发给对方。验证码不包含在链接里，两项凭据只在创建时显示；关闭后需要撤销旧邀请、重新生成。对方打开链接，输入验证码并设置自己的密码，注册后回到登录页登录。用户名由邀请指定，链接只能使用一次，输错验证码 5 次后失效。

没有有效链接和验证码不能注册。管理员可以撤销未使用的邀请，以及停用、重新启用、强制退出、删除账号或生成密码重置链接。敏感操作需要管理员当前密码；删除还需要输入目标用户名，并会删除该账号的业务数据和上传图片。

每个账号的订阅、家庭组、账单、图片、设置和网页备份都独立。账号管理页只提供账号状态与管理操作，不提供查看其他人订阅的入口。拥有服务器和数据库权限的人仍能访问服务器上的数据。

所有用户都可以在「设置 → 账号与安全」修改用户名，需验证当前密码。新用户名立即生效，其他设备需要重新登录；已有记录和头像继续保留。用户名不区分大小写，不能占用其他账号或有效邀请已预留的名字。

邀请和密码重置由网站本身处理，不需要新增 API Key、邮箱或短信服务。
...
支持 CNY、USD、EUR、GBP、HKD、TWD、JPY、KRW、KWD，以及 NGN（尼日利亚奈拉）和 TRY（土耳其里拉）。NGN、TRY 均保留两位小数，也可以作为总额显示币种。
...
- 家庭组成员是当前账号里的分摊档案，不会自动生成登录账号。需要让对方使用自己的空间时，由管理员另行邀请注册；不同账号之间不会共享家庭组数据。
